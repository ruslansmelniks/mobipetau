...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const checkDraft = async () => {
      try {
        // Try querying the pets table (known to work)
        let petsRes = await supabase.from('pets').select('id').limit(1);
        console.log('Pets table query:', petsRes);

        // Try querying appointments table (should fail)
        let apptRes = await supabase.from('appointments').select('id').limit(1);
        console.log('Appointments table query:', apptRes);

        // 4. Both filters (the real draft check)
        ...
        console.log('Both filters:', res);

        if (res.error) {
          // Only show error if it's not a 406 (which means "not acceptable" or "no data")
          if (res.error.code !== '406') {
            setShowDraftBanner(false);
            // Optionally show a less scary error, or just log it
            console.error('Error fetching draft appointment:', res.error);
          }
          // Don't show the error banner for 406
          return;
        }

        const draft = res.data && res.data[0];
        if (draft) {
        ...
        console.error('Unexpected error in checkDraft:', err);
        setShowDraftBanner(false);
      }
    };
    checkDraft();
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
        {showDraftBanner && (
          <div className="bg-yellow-100 border border-yellow-300 text-yellow-800 p-4 rounded mb-6 flex items-center justify-between max-w-3xl mx-auto mt-6">
